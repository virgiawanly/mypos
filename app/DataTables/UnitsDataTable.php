    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                $action = '<a href="' . route('units.edit', $row->id) . '" class="btn btn-success me-1"><i class="bi bi-pencil-fill"></i></a>';
                $action .= '<button data-action="' . route('units.destroy', $row->id) . '" data-name="' . $row->name . '" class="btn btn-danger delete-btn"><i class="bi bi-trash-fill"></i></button>';
                return $action;
            });
    }
